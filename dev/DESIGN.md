...
1. `SR`, `ST`, `Sim.CI` and `Step`, kept faithful to 1.0.0 with fixes. The nodewise tuning is now explicit and defaults to `"cv"`.
...
|---|---|---|---|---|
| Nodewise λ in SR/ST/Sim.CI/Step | Z&Z (hdi ≥ 0.1-7 changed a default under SILM) | Contradicts ZC paper Sec 5 ("10-fold CV") | `nodewise = "cv"` (= SILM 1.0.0 + hdi 0.1-6) | `nodewise = "ZnZ"` (= SILM 1.0.0 + hdi 0.1-7..0.1-10) |
...
- **`# SILM 2.0.0`**, with subsections:
  ...
  - *Behaviour changes (BREAKING)*: the nodewise default, with this history. SILM 1.0.0 was published 2019-01-09 against hdi 0.1-6 and used the 10-fold-CV `lambda.min`. hdi 0.1-7 (2019-03-29) added `do.ZnZ = TRUE` as the default of an internal function SILM called, so from then until archival SILM silently used Z&Z and ignored `lambdatuningfactor = 1`. `nodewise = "ZnZ"` reproduces that behaviour.
...
```sh
cd "/Users/xianyangzhang/My Drive/SILM" && git init -b main
...
| 9 | `feat!: add nodewise argument; default to paper's 10-fold CV lambda` | `BREAKING CHANGE:` footer. E0 in both modes. |
...
```r
SR(X, Y, nodewise = c("cv", "ZnZ"), center = FALSE, Theta = NULL,
   parallel = FALSE, ncores = getOption("mc.cores", 2L))
Sim.CI(X, Y, set, M = 500, alpha = 0.95, nodewise = c("cv", "ZnZ"), center = FALSE,
       Theta = NULL, parallel = FALSE, ncores = getOption("mc.cores", 2L))
Step(X, Y, M = 500, alpha = 0.05, nodewise = c("cv", "ZnZ"), center = FALSE,
     Theta = NULL, parallel = FALSE, ncores = getOption("mc.cores", 2L))
ST(X.f, Y.f, sub.size, test.set, M = 500, alpha = 0.05, nodewise = c("cv", "ZnZ"),
   center = FALSE, legacy = FALSE, parallel = FALSE, ncores = getOption("mc.cores", 2L))
Theta.hat(X, nodewise = c("cv", "ZnZ"), center = FALSE, parallel = FALSE,
...
4. Reproducing old results (`nodewise = "ZnZ"`, `legacy = TRUE`, RNGkind).
...
Planned files:
- `/Users/xianyangzhang/My Drive/SILM/R/nodewise_api.R` (single nodewise entry point with a required `do_znz` and a `foldid`; basis of both the `"cv"`/`"ZnZ"` parity and the hdi parity)
- `/Users/xianyangzhang/My Drive/SILM/R/scaled_lasso.R` (clean-room scaled lasso; must be `identical()` to scalreg 1.0.1)
- `/Users/xianyangzhang/My Drive/SILM/R/boot_lasso_proj.R` (and `boot_refit.R`, `boot_resample.R`: exact hdi RNG order, pre-drawn folds, the DBZ additions)
- `/Users/xianyangzhang/My Drive/SILM/R/ST.R` (the most fixes and the `legacy` gating)
- `/Users/xianyangzhang/My Drive/SILM/validation/run_equivalence.R` (with `setup_legacy_libs.R` and `compare.R`: the live oracle behind every E0 claim)
...
Sources read: the paper PDF /Users/xianyangzhang/Downloads/s11749-017-0554-2.pdf (all 35 pp.) and the hdi 0.1-10 files in  (hdi_boot.lasso-proj.R = "BLP", hdi_helpers.R = "HLP", hdi_methods.R = "MTH", helpers.nodewise.R = "NW").
...
Sources: /Users/xianyangzhang/Downloads/s11749-017-0554-2.pdf and, in , the files hdi_boot.lasso-proj.R, hdi_helpers.R, hdi_methods.R and helpers.nodewise.R, plus the hdi Rd fetched via gh api.
